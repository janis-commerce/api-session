 ...
 * @property {string | undefined} sellerId
 */
...
module.exports = class ApiSession {
	...
	constructor(authenticationData, client) {
		...
		this._client = client;
		/** @private */
		this._useClientCache = true;
	}

	get useClientCache() {
		return this._useClientCache;
	}

	set useClientCache(useClientCache) {
		this._useClientCache = useClientCache;
	}
	...
	/**
	 * Get the client currency code associated to the session
	 *
	 * @return {string|undefined} The currency code of the client or undefined in case there is no client nor currency
	 */
	get currency() {
		return this.authenticationData.currency;
	}

	/**
	 * Get the client currencyDisplay code associated to the session
	 *
	 * @return {string|undefined} The currencyDisplay code of the client or undefined in case there is no client nor currencyDisplay
	 */
	get currencyDisplay() {
		return this.authenticationData.currencyDisplay;
	}
	...
	/**
	 * Get if has Access To All locations associated to the session
	 *
	...
	/**
	 * Get the warehousesIds array associated to the session
	 *
	 * @return {Array<string>|undefined} The warehousesIds or undefined in case there are no warehousesIds associated
	 */
	get warehousesIds() {
		return this.authenticationData.warehousesIds;
	}

	/**
	 * Get the sellerId associated to the session
	 *
	 * @return {string|undefined} The Seller ID or undefined if none is assigned
	 */
	get sellerId() {
		return this.authenticationData.sellerId;
	}
	...
	get client() {
		...
		return ClientHandler
			.getByCode(this.clientCode, this.useClientCache)
	...
	/**
	 * Validate if the session has access to the warehouse
	 *
	 * @param {string} warehouseIds Warehouse Id
	 * @returns {Boolean}
	 */
	validateWarehouse(warehouseIds) {
		return !!this.hasAccessToAllLocations || (!!warehouseIds && Array.isArray(this.warehousesIds) && this.warehousesIds.includes(warehouseIds));
	}
};
